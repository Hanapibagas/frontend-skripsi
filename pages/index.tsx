...
const A = () => {
  ...
  const addAudioElement = async (blob: Blob) => {
    try {
      ...
      try {
        if (posting.data.encoded_text && posting.data.mapsLink && posting.data.nama_pengirim) {
          const encodedText = posting.data.encoded_text;
          const mapsLink = encodeURIComponent(posting.data.mapsLink);
          const namaPengirim = encodeURIComponent(posting.data.nama_pengirim);

          const response = await axios.get(`http://localhost:8000/mitigasi/?msg=${encodedText}&lokasi=${mapsLink}&nama=${namaPengirim}`);
          ...
          const laporanData = {
            nama_laporan: posting.data.nama_pengirim,
            isi_laporan: posting.data.text,
            kelas: kelas,
            alamat: posting.data.mapsLink
          };
    ...
    <div className="w-full h-screen flex items-center justify-center bg-gray-200 ">
      <div className="md:max-w-1xl h-full lg:max-w-2xl duration-500 xl:max-w-xl w-full flex flex-col items-stretch grow flex-shrink-0 gap-x-4 md:p-2 py-2">
        <div className="bg-white w-full rounded-xl border border-gray-300 flex flex-col h-full items-center justify-between">
          <div className="h-32 flex justify-between items-center w-full px-8">
